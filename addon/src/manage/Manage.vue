...
if (!BG.inited) {
	browser.runtime.onMessage.addListener(
		({ action }) => "i-am-back" === action && window.location.reload()
	);
...
export default {
	data() {
		return {
			...
			// mta
			tabSearchString: null,
			foundTabs: [],
			groupToAddTabsTo: null,
			groupToEdit: null,
			groupToRemove: null,
	...
	watch: {
		...
		"options.showTabsWithThumbnailsInManageGroups": function(
			value,
			oldValue
		) {
	...
	computed: {
		filteredGroups() {
			...
			if (!searchStr) {
				return this.groups.map(
					(group) => ((group.filteredTabs = group.tabs), group)
				);
			}

			return this.groups.map((group) => {
				group.filteredTabs = group.tabs.filter((tab) =>
					utils.mySearchFunc(
						searchStr,
		...
		filteredUnSyncTabs() {
			...
			return this.unSyncTabs.filter((tab) =>
				utils.mySearchFunc(
					searchStr,
					utils.getTabTitle(tab, true),
					this.extendedSearch
		...
		isCurrentWindowIsAllow() {
			return (
				this.currentWindow && utils.isWindowAllow(this.currentWindow)
			);
		},
		currentGroup() {
			...
			return this.groups.find(
				(group) => group.id === this.currentWindow.groupId
			);
		},
	},
	methods: {
		...
		setupListeners() {
			this.$on("drag-move-group", function(from, to) {
				Groups.move(
					from.data.item.id,
					this.groups.indexOf(to.data.item)
				);
			})
				.$on("drag-move-tab", function(from, to) {
					if ("new-group" === to.data.item.id) {
						...
						let tabIds = this.getTabIdsForMove(),
							groupId = this.isGroup(to.data.item)
								? to.data.item.id
								: to.data.group.id,
							index = this.isGroup(to.data.item)
								? undefined
								: to.data.item.index;
				...
				.$on(
					"drag-moving",
					(item, isMoving) => (item.isMoving = isMoving)
				)
			...
			const removeTab = (tabId, withAllTabs = false) => {
				...
				lazyRemoveTabTimer = setTimeout(
					(tabIds) => {
						lazyRemoveTabIds = [];

						this.multipleTabIds = this.multipleTabIds.filter(
							(tabId) => !tabIds.includes(tabId)
						);
						this.unSyncTabs = this.unSyncTabs.filter(
			...
			const lazyAddTab = (tab, groupId) => {
				...
				let group = groupId
					? this.groups.find((gr) => gr.id === groupId)
					: null;

				if (group) {
					if (!Object.isFrozen(group.tabs)) {
						let index = group.tabs.findIndex(
							(t) => t.index === tab.index
						);
					...
					lazyAddUnsyncTabTimer = setTimeout(
			...
			const onCreatedTab = (tab) => {
				...
				lazyCreateTabsTimer = setTimeout(
					function(tabs) {
						lazyCreateTabs = [];

						tabs.forEach((tab) =>
							lazyAddTab(tab, cache.getTabGroup(tab.id))
						);
					},
					200,
					lazyCreateTabs
			...
			const onUpdatedTab = (tabId, changeInfo, tab) => {
				...
				if (
					...
					changeInfo.hasOwnProperty("hidden")
				) {
			...
			const onMovedTab = (tabId, { windowId }) => {
				...
				if (groupId) {
					...
					onMovedTabTimer = setTimeout(
						(groupId) => this.loadGroupTabs(groupId),
						100,
						groupId
					);
				} else {
					clearTimeout(onMovedUnsyncTabTimer);
					onMovedUnsyncTabTimer = setTimeout(
						() => this.loadUnsyncedTabs(),
						100
					);
				}
			...
			const onDetachedTab = (tabId, { oldWindowId }) => {
				// notice: call before onAttached
			...
			const onAttachedTab = (tabId, { newWindowId }) => {
				...
				onAttachedTabWinTimer = setTimeout(() =>
				...
				if (groupId) {
					...
					onAttachedUnsyncTabTimer = setTimeout(
						() => this.loadUnsyncedTabs(),
						100
					);
				}
			...
			const onMessage = async (request) => {
				switch (request.action) {
					...
					case "containers-updated":
						...
						Object.values(this.allTabs).forEach(
							this.mapTabContainer
						);
						break;
					case "lock-addon":
						this.isLoading = true;
			...
			if (!this.isCurrentWindowIsAllow) {
				window.addEventListener("resize", function() {
					if (
						window.localStorage.manageGroupsWindowWidth !=
						window.innerWidth
					) {
						window.localStorage.manageGroupsWindowWidth =
							window.innerWidth;
					...
					if (
						window.localStorage.manageGroupsWindowHeight !=
						window.innerHeight
					) {
						window.localStorage.manageGroupsWindowHeight =
							window.innerHeight;
		...
		async moveTabToNewGroup(tabId, loadUnsync, showTabAfterMoving) {
			...
			if (this.options.alwaysAskNewGroupName) {
				...
				newGroupTitle = await this.showPrompt(
					this.lang("createNewGroup"),
					newGroupTitle
				);
		...
		mapGroup(group) {
			if (group.isArchive) {
				group.tabs = Object.freeze(
					group.tabs
						.map(utils.normalizeTabFavIcon)
						.map(this.mapTabContainer)
				);
			} else {
		...
		mapTab(tab) {
			Object.keys(tab).forEach(
				(key) => !availableTabKeys.has(key) && delete tab[key]
			);
		...
		moveTabsToGroup(group, searchString) {
			let tabs = this.getTabsToMove(group, searchString);
			let tabIds = tabs.map((tab) => tab.id);
			let index = undefined;
			let groupId = group.id;
			BG.Tabs.move(tabIds, groupId, index, false);
			this.groupToAddTabsTo = null;
		},

		updateFoundTabs(group, searchString) {
			const tabs = this.getTabsToMove(group, searchString);
			this.$set(this, "foundTabs", tabs);
		},

		getTabsToMove(group, searchString) {
			let groupId = group.id;
			let tabs = this.searchForTabs(groupId, searchString);
			return tabs;
		},
		...
		discardOtherGroups(groupExclude) {
			let tabIds = this.groups.reduce(function(acc, gr) {
				let groupTabIds =
					gr.id === groupExclude.id ||
					gr.isArchive ||
					cache.getWindowId(gr.id)
						? []
						: gr.tabs.map(utils.keyId);
		...
		async clickOnTab(event, tab, group) {
			if (event.ctrlKey || event.metaKey) {
				if (this.multipleTabIds.includes(tab.id)) {
					this.multipleTabIds.splice(
						this.multipleTabIds.indexOf(tab.id),
						1
					);
				} else {
				...
				if (this.multipleTabIds.length) {
					let tabIds = group
							? group.filteredTabs.map(utils.keyId)
							: this.filteredUnSyncTabs.map(utils.keyId),
					...
					this.multipleTabIds
						...
						.some(function(tabId) {
							return (
								-1 !== (lastTabIndex = tabIds.indexOf(tabId))
							);
						});
					...
					if (-1 === lastTabIndex) {
						...
						let multipleTabIndex = this.multipleTabIds.indexOf(
							tabIds[lastTabIndex]
						);

						for (
							let i = Math.min(tabIndex, lastTabIndex),
								maxIndex = Math.max(tabIndex, lastTabIndex);
							i <= maxIndex;
							i++
						) {
		...
		dragHandle(event, itemType, allowTypes, data) {
			if (
				...
				(!this.dragData || !this.dragData.allowTypes.includes(itemType))
			) {
				return;
			}
			...
			switch (event.type) {
				case "dragstart":
					...
					if ("tab" === itemType) {
						...
						if (1 < this.multipleTabIds.length) {
							let multiTabsNode = document.getElementById(
								"multipleTabsText"
							);
							multiTabsNode.innerText = browser.i18n.getMessage(
								"movingMultipleTabsText",
								this.multipleTabIds.length
							);

							event.dataTransfer.setDragImage(
								multiTabsNode,
								20,
								multiTabsNode.offsetHeight - 80
							);
						}
					}
					break;
				...
				case "drop":
					...
					if (data.item !== this.dragData.data.item) {
						this.$emit(
							"drag-move-" + this.dragData.itemType,
							this.dragData,
							{ itemType, allowTypes, data }
						);
					}
	...
	>
		<header class="is-flex is-align-items-center">
			<span class="page-title">
				<span v-text="lang('extensionName')"></span> -
			...
			<span class="is-full-width">
				<div>
					<label class="checkbox">
						...
						<span
							v-text="
								lang('showTabsWithThumbnailsInManageGroups')
							"
						></span>
					</label>
				</div>
				<div class="buttons has-addons" style="display: none;">
					<span
						...
						v-text="lang('manageGroupViewGrid')"
					></span>
					<span
						class="button is-small"
						disabled
						v-text="lang('manageGroupViewFreeArrange')"
					></span>
				</div>
			</span>
			<span>
				...
				>
					...
					<div
						...
						@contextmenu="
							'INPUT' !== $event.target.nodeName &&
								!group.isArchive &&
								$refs.groupContextMenu.open($event, { group })
						"
						:draggable="String(group.draggable)"
						@dragstart="
							dragHandle($event, 'group', ['group'], {
								item: group,
							})
						"
						@dragenter="
						...
						<div class="header">
							...
							>
								<figure class="image is-16x16">
									<img
										:src="
											containers[group.newTabContainer]
												.iconUrl
										"
										:style="{
											fill:
							...
							></div>
							<div
								class="group-icon cursor-pointer is-unselectable"
							...
							<div
								...
								:class="[
									'tab',
									{
										...
										'is-in-multiple-drop': multipleTabIds.includes(
											tab.id
										),
										'has-thumbnail':
											options.showTabsWithThumbnailsInManageGroups &&
								...
								@contextmenu.stop.prevent="
									!group.isArchive &&
										$refs.tabsContextMenu.open($event, {
											tab,
											group,
										})
								"
							...
							>
								...
								>
									...
									>
										...
										<img
											v-else
											class="size-16"
											v-lazy="tab.favIconUrl"
										/>
									</div>
									<div
										v-if="isTabLoading(tab)"
										class="refresh-icon"
										:style="
											tab.container
												? {
								...
								<template v-if="tab.container">
									...
									>
										<img
											class="size-16"
											:src="tab.container.iconUrl"
											:style="{
												fill: tab.container.colorCode,
											}"
										/>
									</div>
								</template>
								...
								>
									<img
										v-if="tab.thumbnail"
										v-lazy="tab.thumbnail"
									/>
								</div>
								<div
									@mousedown.middle.prevent
									@mouseup.middle.prevent="
										!group.isArchive && removeTab(tab)
									"
									...
									:style="
										tab.container
											...
											: false
									"
									v-text="getTabTitle(tab, false, 0, true)"
								></div>
					...
					<div v-if="unSyncTabs.length" class="group">
						<div class="header">
							...
							<div
								class="tabs-count"
								v-text="groupTabsCountMessage(unSyncTabs)"
							></div>
						</div>
						<div
							:class="[
						...
						>
							<div
								...
								:title="getTabTitle(tab, true)"
								@contextmenu.stop.prevent="
									$refs.tabsContextMenu.open($event, { tab })
								"
					...
					<div
						class="group new cursor-pointer"
						@click="addGroup"
						draggable="true"
						@dragover="dragHandle($event, 'tab', ['tab'])"
						@drop="
							dragHandle($event, 'tab', ['tab'], {
								item: { id: 'new-group' },
							})
						"
					>
						<div class="body">
							...
							<div
								class="h-margin-top-10"
								v-text="lang('createNewGroup')"
							></div>
						</div>
					</div>
				</div>
		...
		<context-menu ref="groupContextMenu">
			<template v-slot="menu">
				<ul v-if="menu.data" class="is-unselectable">
					...
					<li
						v-if="menu.data.group !== currentGroup"
						@click="discardGroup(menu.data.group)"
					>
						<img src="/icons/snowflake.svg" class="size-16" />
						<span
							v-text="lang('hotkeyActionTitleDiscardGroup')"
						></span>
					</li>
					<li
						v-if="groups.length > 1"
						@click="discardOtherGroups(menu.data.group)"
					>
						<img src="/icons/snowflake.svg" class="size-16" />
						<span
							v-text="lang('hotkeyActionTitleDiscardOtherGroups')"
						></span>
					</li>
					...
					<li
						...
						v-if="
							container.cookieStoreId !==
								...
								(menu.data.group.ifDifferentContainerReOpen
									? menu.data.group.excludeContainersForReOpen.includes(
											container.cookieStoreId
									  ) ||
									  menu.data.group.newTabContainer ===
											container.cookieStoreId ||
									  container.cookieStoreId ===
											TEMPORARY_CONTAINER
									: true)
						...
						@click="
							addTab(menu.data.group, container.cookieStoreId)
						"
					>
						<img
							v-if="container.iconUrl"
							:src="container.iconUrl"
							class="is-inline-block size-16"
		...
		<context-menu ref="tabsContextMenu">
			<template v-slot="menu">
				<ul v-if="menu.data" class="is-unselectable">
					...
					<li
						v-if="multipleTabIds.length"
						@click="removeTab(menu.data.tab)"
					>
						<img src="/icons/close.svg" class="size-16" />
						<span v-text="lang('deleteTab')"></span>
					</li>
					<li
						v-if="!menu.data.tab.discarded"
						@click="discardTab(menu.data.tab)"
					>
						<img src="/icons/snowflake.svg" class="size-16" />
					...
					<li
						v-if="
							menu.data.group && menu.data.group !== currentGroup
						"
						@click="discardGroup(menu.data.group)"
					>
						<img src="/icons/snowflake.svg" class="size-16" />
					...
					<li
						v-if="menu.data.group && groups.length > 1"
						@click="discardOtherGroups(menu.data.group)"
					>
						<img src="/icons/snowflake.svg" class="size-16" />
						<span
					...
					<li
						v-if="options.showTabsWithThumbnailsInManageGroups"
						@click="updateTabThumbnail(menu.data.tab)"
					>
					...
					<li class="is-disabled">
						<img class="size-16" />
						<span
							v-text="lang('moveTabToGroupDisabledTitle') + ':'"
						></span>
					</li>
		...
		<popup
			...
			@close-popup="groupToAddTabsTo = null"
			@save-group="() => {
		...
		>
			<span
				v-html="lang('deleteGroupBody', safeHtml(groupToRemove.title))"
			></span>
		</popup>
		...
		>
			<div class="control is-expanded">
				<input
					...
					class="input"
					ref="promptInput"
					@keyup.enter.stop="promptResolveFunc(true)"
				/>
			</div>
		</popup>
	...
	.group {
		...
		> .body:not(.in-list-view) > .tab {
			...
			&:not(.has-thumbnail) > .tab-icon {
				...
				height: 16px;
				top: calc(
					(calc(100% - 1em - var(--tab-inner-padding)) / 2) - 8px
				);
				left: calc((100% / 2) - 8px);
			}
		...
		> .body.in-list-view > .tab {
			...
			&.new {
				justify-content: center;
				border: var(--tab-border-width) dashed
					var(--tab-inner-border-color);
