(function () {
	...
	async function moveSilly(tabIds, groupId, newTabIndex = -1) {
		await move(tabIds, groupId, -1, true, false, true);
	}

	async function move(tabIds, groupId, newTabIndex = -1, showNotificationAfterMoveTab = true, showTabAfterMoving = false, skipSelfCheck = false) {
		tabIds = tabIds.slice();
